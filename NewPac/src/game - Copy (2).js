...
TestGame.Game.prototype = {
    ...
    update: function(){
        ...
        if (this.bGameRun)
        {
            ...
            this.game.physics.collide(pacman, layer, this.pacTileHit, null, this);
            ...
            {
                pacDirectionCurrent = DirectionEnum.UP;
            }
            else if (cursors.down.isDown)
            {
                pacDirectionCurrent = DirectionEnum.DOWN;
            }
            if (cursors.left.isDown)
            {
                pacDirectionCurrent = DirectionEnum.LEFT;
            }
            else if (cursors.right.isDown)
            {
                pacDirectionCurrent = DirectionEnum.RIGHT;
            }
            
            //Lets use the direction to move our little guy
            if (pacDirectionCurrent == DirectionEnum.RIGHT)
            {
                pacAnimPlay = pacAnimPrefix + 'right';
                pacMoveX = pacVelocity;
                pacMoveY = 0;
            }
            else if (pacDirectionCurrent == DirectionEnum.LEFT)
            {
                pacAnimPlay = pacAnimPrefix + 'left';
                pacMoveX = pacVelocity * -1;
                pacMoveY = 0;
            }
            else if (pacDirectionCurrent == DirectionEnum.UP)
            {
                pacAnimPlay = pacAnimPrefix + 'rightup';
                pacMoveX = 0;
                pacMoveY = pacVelocity * -1;
            }
            else if (pacDirectionCurrent == DirectionEnum.DOWN)
            {
            ...
            if (pacDirectionCurrent != pacDirectionLast)
            {
                pacChangeDirection = true;
            }
        }
    ...
    pacTileHit: function(pacObj, layerObj) {
        if (this.bGameRun)
        {
            ...
            console.log('pacTileHit');
    
            var collidingTile = map.getTile(layerObj.getTileX(pacObj.x), layerObj.getTileY(pacObj.y));
            var collidingTileX = layerObj.getTileX(pacObj.x);
            var collidingTileY = layerObj.getTileY(pacObj.y);
            
            //Get colliding tile
            if (pacObj.body.touching.left)
            {
                collidingTile = map.getTile(layerObj.getTileX(pacObj.x) - 1, layerObj.getTileY(pacObj.y));
                collidingTileX = layerObj.getTileX(pacObj.x) - 1;
                collidingTileY = layerObj.getTileY(pacObj.y);
            }
            ...
            else if (pacObj.body.touching.up)
            {
                ...
                collidingTileX = layerObj.getTileX(pacObj.x);
                collidingTileY = layerObj.getTileY(pacObj.y) - 1;
            }
            ...
            {
                //Eating dot
                map.putTile(0, collidingTileX, collidingTileY, 0);
    ...
    render: function() {
        if (this.bGameRun)
        {
            if ((pacman.x % 32 == 0 && pacman.y % 32 ==0) && (pacChangeDirection))
            {
                pacman.animations.play( pacAnimPlay, 5, true);
                pacman.body.velocity.x = pacMoveX;
                pacman.body.velocity.y = pacMoveY;
                pacDirectionLast = pacDirectionCurrent;
                pacChangeDirection = false;
            }
            else
            {
                pacman.body.velocity.x = pacMoveX;
        ...
        this.game.debug.renderText('pacDirectionCurrent: ' + pacDirectionCurrent, 900, 630, 'rgb(255,255,255)');
        this.game.debug.renderText('pacChangeDirection: ' + pacChangeDirection.toString(), 900, 645, 'rgb(255,255,255)');
